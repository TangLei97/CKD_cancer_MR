...
library(data.table)
library(dplyr)
wuttke_2019_snp <- readxl::read_xlsx('~/Desktop/ckd cancer/ivs/kidney/Wuttke2019-supplement-Tables.xlsx',sheet = 12) %>% 
  filter(p_value < 5e-08)
#wuttke_2019_snp <- mutate(wuttke_2019_snp,chr=matrix(unlist(strsplit(`Chr/Pos (b37)`,split = ':')),byrow = T,ncol = 2)[,1])
#Summary data
wuttke_2019_sum <- fread('~/Desktop/ckd cancer/summary data/kfun.txt')
...
library(dplyr)
teumer_2019_snp <- readxl::read_xlsx('~/Desktop/ckd cancer/ivs/kidney/Teumer2019-supplement-3.xlsx') %>% 
  filter(`P-value MA` < 5e-08)
#Summary data
teumer_2019_sum <- fread('~/Desktop/ckd cancer/summary data/ma.gz', fill = T)
